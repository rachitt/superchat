...
export async function streamAiChat(opts: StreamAiChatOptions): Promise<any> {
  return withSpan("ai.streamChat", async () => {
    return _streamAiChatInner(opts);
  }, {
    "ai.channelId": opts.channelId,
    "ai.model": "auto",
    "ai.maxSteps": opts.tools ? AI_MAX_AGENT_STEPS : 0,
  });
}

async function _streamAiChatInner(opts: StreamAiChatOptions): Promise<any> {
