  ...
  const markdownComponents = useMemo(
    () => ({
      ...
      code: ({ children, className, ...props }: React.ComponentProps<"code">) => {
        ...
        return isInline ? (
          <code className="rounded-md bg-teal-500/10 px-1.5 py-0.5 text-[13px] font-mono text-teal-300" {...props}>
            {children}
          </code>
        ) : (
      ...
      ul: ({ children, ...props }: React.ComponentProps<"ul">) => (
        <ul className="mb-2 ml-4 list-disc last:mb-0 marker:text-teal-400/50" {...props}>{children}</ul>
      ),
      ol: ({ children, ...props }: React.ComponentProps<"ol">) => (
        <ol className="mb-2 ml-4 list-decimal last:mb-0 marker:text-teal-400/50" {...props}>{children}</ol>
      ),
      a: ({ children, ...props }: React.ComponentProps<"a">) => (
        <a className="text-teal-400 underline decoration-teal-400/30 hover:decoration-teal-400 transition-colors" target="_blank" rel="noopener noreferrer" {...props}>
          {children}
        </a>
      ),
  ...
  return (
    <div className="group relative flex gap-3 px-5 py-2">
      ...
      <Avatar className="h-9 w-9 shrink-0">
        <AvatarFallback className="bg-gradient-to-br from-teal-600 to-teal-600 text-foreground">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-semibold text-teal-400">{AI_BOT_NAME}</span>
          <Badge variant="secondary" className="h-4.5 bg-teal-500/10 px-1.5 text-[10px] font-medium text-teal-400 border-teal-500/20">
            AI
          </Badge>
        </div>
