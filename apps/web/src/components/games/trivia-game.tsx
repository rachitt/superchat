...
export function TriviaGame({ game, players, userId, onAction }: TriviaGameProps) {
  ...
  if (!currentQuestion) return <div className="p-4 text-zinc-400">Loading...</div>;
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-zinc-500 uppercase">Q {state.currentQuestionIndex + 1}/{state.questions.length}</span>
        <div className={`rounded-full px-3 py-1 text-sm font-bold ${timeLeft <= 5 ? "bg-red-900/30 text-red-400" : "bg-zinc-800 text-zinc-300"}`}>⏱ {timeLeft}s</div>
      </div>
      ...
      <div className="space-y-2">
        {currentQuestion.options.map((opt, idx) => {
          ...
          else if (isSelected) cls += "border-indigo-500 bg-indigo-900/30 text-indigo-300";
          else if (hasAnswered || isReveal) cls += "border-zinc-700 bg-zinc-800/30 text-zinc-500";
          else cls += "border-zinc-700 bg-zinc-800 text-zinc-200 hover:border-zinc-500 cursor-pointer";
          return (
            <button key={idx} onClick={() => handleAnswer(idx)} disabled={hasAnswered || isReveal} className={cls}>
              <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded text-xs font-bold ${isCorrect ? "bg-emerald-600 text-white" : isWrong ? "bg-red-600 text-white" : isSelected ? "bg-indigo-600 text-white" : "bg-zinc-700 text-zinc-300"}`}>{String.fromCharCode(65 + idx)}</span>
              {opt}
            </button>
          );
        })}
      </div>
      {hasAnswered && !isReveal && <p className="text-center text-sm text-zinc-400">Waiting for others...</p>}
    ...
    <div className="rounded-lg border border-zinc-700/50 bg-zinc-800/30 p-3 mt-2">
      ...
      {players.map((p) => (
        <div key={p.userId} className="flex items-center justify-between py-0.5">
          <span className={`text-sm ${p.userId === userId ? "text-indigo-400 font-medium" : "text-zinc-400"}`}>{p.displayName || p.username}{p.userId === userId ? " (you)" : ""}</span>
          <span className="text-sm font-mono text-zinc-300">{scores[p.userId] || 0}</span>
        </div>
      ))}
    </div>
  );
