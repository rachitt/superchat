  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4">
      ...
      <div className="flex items-center gap-6 text-sm">
        <div className={state.currentTurnUserId === state.players.x ? "opacity-100" : "opacity-50"}><span className="font-black text-indigo-400">X</span> {xP?.displayName || "P1"}</div>
      ...
      <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${state.boardSize}, 1fr)` }}>
        {state.board.map((cell, i) => {
          ...
          else if (sym === "X") cls += "border-indigo-600/50 bg-indigo-900/20 text-indigo-400";
          else if (sym === "O") cls += "border-pink-600/50 bg-pink-900/20 text-pink-400";
          else if (isMyTurn && !isOver) cls += "border-zinc-700 bg-zinc-800 hover:border-zinc-500 cursor-pointer";
          else cls += "border-zinc-700/50 bg-zinc-800/50";
