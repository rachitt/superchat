...
import { XpProgress } from "../gamification/xp-progress";
...
export function WorkspaceSidebar({ workspaceId, workspaceName, channels }: WorkspaceSidebarProps) {
  ...
  // Fetch user stats to populate gamification store
  const trpc = useTRPC();
  const { data: me } = useQuery(trpc.user.me.queryOptions());
  useEffect(() => {
    if (me) {
      setStats(me.xp, me.level, me.streakDays);
    }
  }, [me, setStats]);
            ...
            <div className="flex flex-col items-center gap-1 px-1">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    onClick={() => setShowLeaderboard(true)}
                    className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-accent hover:text-amber-400"
                  >
                    <Trophy className="h-4 w-4" />
                  </button>
                </TooltipTrigger>
